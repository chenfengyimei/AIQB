...
const { fetchText, isPublicIp, resolvePublicAddress } = require('./safe-fetch');
...
async function assertPublicDns(urlValue) {
  await resolvePublicAddress(urlValue);
}
...
class EndpointRegistry {
  constructor(dataDir, options) { this.dir = path.join(dataDir, 'endpoints'); this.configFile = path.join(this.dir, 'config.json'); this.stateFile = path.join(this.dir, 'state.json'); this.cacheDir = path.join(this.dir, 'cache'); this.transport = options && options.transport || null; this.explicitPreset = endpointPreset(options && options.preset); this.environmentPreset = endpointPreset(process.env.AIQB_ENDPOINT_PRESET); this.preset = 'community'; this.defaults = COMMUNITY_ENDPOINTS; this.endpoints = []; this.byId = new Map(); this.state = { updatedAt: null, byId: {}, runs: [] }; }
  init() {
  ...
  async _fetch(endpoint, context) { const url = this._resolveProbeUrl(endpoint, context || {}); const started = Date.now(); const response = await (this.transport || fetchText)(url, { headers: { 'User-Agent': UA, 'Accept': endpoint.format === 'atom' || endpoint.format === 'rss' ? 'application/atom+xml, application/rss+xml, application/xml;q=0.9, text/xml;q=0.8' : 'application/json' }, timeoutMs: endpoint.timeoutMs, maxBytes: MAX_RESPONSE_BYTES, maxWireBytes: MAX_RESPONSE_BYTES, maxRedirects: 3 }); const raw = response.body || ''; let payload = raw; let items = null; if (endpoint.role === 'source') { if (endpoint.format === 'json') { payload = raw ? JSON.parse(raw) : null; items = parseJson(payload, endpoint); } else items = parseFeed(raw, endpoint); } else { try { payload = raw ? JSON.parse(raw) : null; } catch (error) {} } return { response, raw, payload, items, durationMs: Date.now() - started }; }
